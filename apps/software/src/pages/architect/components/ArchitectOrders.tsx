...
import { viewArchitectAtom, ViewArchitectType } from "@/store/architect";
...
const ArchitectOrders = () => {
  ...
  return (
    <div className="mt-2">
      ...
      <ArchitectOrderTable
        caption="A list of architect's Active orders."
        orders={viewArchitect.orders.filter(
          (order) => order.status === "Pending"
        )}
      />
      <span className="font-sofiapro font-bold text-xl">Completed Orders</span>
      <ArchitectOrderTable
        caption="A list of architect's Completed orders."
        orders={viewArchitect.orders.filter(
...
export const ArchitectOrderTable = ({
  caption,
  orders,
}: {
  caption: string;
  orders: ViewArchitectType["orders"];
}) => {
